...
namespace Extended
{
    public static class Utility
    {
        ...
        public static int GetIndexRowMaxSum<T>(this T[,] matrix) where T : IAdditionOperators<T, T, T>, IComparable<T>
        {
            int index = 0;

            for (int i = 1; i < matrix.GetLength(0); i++)
            {
                if (matrix.GetSumRow(index).CompareTo(matrix.GetSumRow(i)) < 0)
                    index = i;
            }
        ...
        public static IEnumerable<(char, int)> GetFrequencyDictionary(this string text, bool ignoreCase = false)
        {
            return text
                    .Select(e => { if (ignoreCase) return char.ToLower(e); else return e; })
                    .GroupBy(c => c)
                    .Select(g => (g.Key, g.Count()))
                    .OrderByDescending(t => t.Item2).ThenBy(t => t.Item1);
        }

        public static void DisplayFrequencyDictionary(this string text)
        {
            foreach (var item in text.GetFrequencyDictionary())
            {
                Console.WriteLine($"Символ {item.Item1} встречается {item.Item2} раз. Частота {(1.0 * item.Item2 / text.Length):F3} %");
            }
